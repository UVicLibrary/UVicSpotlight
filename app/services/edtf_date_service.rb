  ...
  # (https://github.com/duke-libraries/edtf-humanize)
    ...
    def first_solr_date
      if [EDTF::Interval, EDTF::Decade, EDTF::Century, EDTF::Season].include?(@parsed_date.class)
        if @parsed_date.class == EDTF::Interval && @parsed_date.open?
          @parsed_date.from.strftime("%F")
        else
          @parsed_date.first.strftime("%F")
        end
    ...
    private
      ...
      def parse_date(date_string)
        if date_string.start_with? "~"
          raise InvalidEdtfDateError.new("Tilde (~) should go at the end of approximate dates instead of the start")
        end
      ...
      def parse_interval(date_string)
        ...
        # Use first_date.first and last_date.last to avoid ArgumentError: Intervals cannot start with: ...
      ...
      # Make dates before 1000 approximate or uncertain if appropriate. Without this,
      # #humanize won't produce the expected result.
